...
#Melt the data to create 2 columns:parameter names and values to be used in below analysis. (This is redundant from previous chunks but included here so I don't need to worry about running other code.)
...
tiff("Figures\\EcoEng\\reg-traj-N2-SOD", width = 350, height = 250, units = "mm", res = 300, compression = "lzw")
...
annual.SOD.model <- lm(value ~ poly(Age, 2), melted.SOD)
newSOD <- c(0, 500, 1000, 1500, 2000, 2500)
predict(annual.SOD.model, data = testSet, type = "response")

filtered <- melted.SOD[which(melted.SOD$Season == "Summer"),]
gg1 <- lm(filtered$value ~ poly(filtered$Age, 2))
...
fig1.SOD <- melted.SOD %>%
  ...
  ggplot() +
  geom_point(aes(Age, value), size = 2) +
  geom_smooth(aes(Age, value), method = "lm", formula = quad.formula, se = F, size = 1, colour = "mediumblue") +
  geom_hline(yintercept = 0, size = 0.2, colour = "grey23") +
  geom_vline(xintercept = 0, size = 0.2, colour = "grey23") +
  theme(panel.background = element_rect(fill = "white", colour = NA),
       axis.title.x = element_blank(),
...
fig1.SOD <- ggplot(melted.SOD) +
  ...
  theme(panel.background = element_rect(fill = "white", colour = NA),
       ...
       axis.ticks.x = element_blank()) +
    ylab(expression(paste("SOD (", mu, "mol ", O[2], " ", m^-2, " ", h^-1, ")"))) +
  geom_point(filtered, aes(Age, value))


#Outer parentheses are a trick for plotting data from two different data sources on the same graph
(p <- ggplot(NULL) +
  geom_smooth(data = melted.SOD, aes(Age, value), method = "lm", formula = quad.formula) +
  geom_smooth(data = filtered, aes(Age, value), method = "lm", formula = quad.formula))

fig1.SOD + geom_line(data = gg1, size = 1)

melted.SOD %>%
  filter(Season == "Summer") +
  fig1.SOD + 
  geom_smooth(aes(Age, value), colour = "purple", method = "lm", formula = quad.formula)
...
tiff("\\Figures\\EnvEng\\N2-flux-vs-SOD", width = 190, height = 380, units = "mm", res = 300, compression = "lzw")

annual.data %>%
  ggplot(aes(SOD, N2Flux, colour = Season)) +
  geom_hline(yintercept = 0, size = 0.7, colour = "black") +
  geom_point(size = 4) +
  theme(panel.background = element_rect(fill = "white", colour = NA),
       axis.title.x = element_text(margin = margin(t = 15), size = 28),
       axis.title.y = element_text(margin = margin(r = 15), size = 28),
