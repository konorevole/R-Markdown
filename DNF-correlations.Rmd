...
bartlett.test(logitSOM ~ Age, annual.fix)

test.norm.logitSOM <- annual.fix %>%
    group_by(Site) %>%
...
Visualizing regressions with transformed data (as transformed in assumption.checks chunk). Includes code to obtain stats for respective regression. Need to run the assumption.checks chunk first to create dataframe with appropriate transformed values.
```{r transformed.regressions}

#Begin by creating new data frame with age as a numerical value rather than factor, which is relevant for regressing all of the transformed data
trial <- add.num.age(annual.fix)

#Set regression formula: 2nd order polynomial
...
#Visualization of the untransformed data for comparison
trial %>%
...
#Regressing the log transformed data. (Also transformed to Box Cox above, so can manipulate below code if visualization is desired.) Pretty much same shape of regression curve but units are VERY different. p value < 0.05 and R2 = 0.16.
trial %>%
...
#Untransformed data for comparison
trial %>%
...
#Regressing the transformed data, respectively. Transformed curve is more pronounced with plateau and units are very different. p < 0.05 and R2 = 0.48.
trial %>%
...
#Regressing untransformed data for comparison. Transformed data gives better R2 (0.47 vs 0.39)-- **altho might be worth running an ANOVA to check if the difference is significant (see saved R-Bloggers page for info)**. p still < 0.05.
trial %>%
  ggplot(aes(NumAge, SOM)) +
  geom_point() +
  geom_smooth(method = "lm", formula = quad.formula)

model4 <- lm(trial$SOM ~ poly(trial$NumAge, 2))
summary(model4)

### SPRING BULK DENSITY ###
#Never able to successfully transform this to meet assumptions, so this is just a visualization of the untransformed data. It doesn't look great (large SE), but stats are ok: p < 0.05 and R2 = 0.44.

#Add numerical age category to sed.data
...
#Summary of transformations: Probably not worth transforming data. Although there are still some assumption violations, the transformations generally do not change the shape of the curve. The data spread is generally tighter, which is good, but the units are completely meaningless. It's probably sufficient to have looked into the potential influence of transformations and to be able to say with some measure of statistical rigor that it doesn't make logical sense. Of course, if reviewers request that I use transformed data instead, the code is all here and ready to go.
#Also, fitting the data (transformed or not) with an ln curve was not successful: it was basically a straight line. Better to stick with 2nd order polynomial which is not intuitively what would be expected, but is what fits better. There is also precedent in the literature- see Menedely docs.
...
**Note: This is the original code for regressions, so it's probably not as useful to spend much time on reviewing. See the code exploring data transformations or the figure revision codes for more informed ways to look at this.**
```{r regression}
...
Old code for exploring regressions that will probably ultimately be deleted. Keeping it for now just in case.
```{r old.work}

#Old attempts to fit regression curves to data
...
model6 <- lm(annual.data$SOD ~ poly(annual.data$Age, 2))
summary(model6)
...
###These were the original attempts to create regressions.
...
Figures revised for Ecological Engineering manuscript. Added extra code so this only requires standard reading in data and pre-treating; do not need to run other code in previous chunks.
```{r manuscript.figs}

#Melt the data to create 2 columns:parameter names and values to be used in below analysis. (Run this first in this chunk. This is redundant from previous chunks but included here so I don't need to worry about running other code.)
melted.all <- melt(annual.data, measure.vars = c("O2conc", "SOD", "PercentOM", "PercentInundation", "NOxFlux", "NH3Flux", "N2Flux"))

quad.formula <- y ~ poly(x,2)

####Restoration trajecory of SOD and N2 flux
#Requires a round-about method because want to include a regression line for summer results for N2 flux only, and also a regression line for all results (including the summer). This requires creating a useless graph just to extract a useful legend, similar to for the habitat breakdown in thesis.revisions. Made the two plots, created a legend, and combined them all using grobs before exporting.

#Starting with N2 flux, which is extra complicated b/c I'm overlaying a regression line from just the summer data, which requires graphing two data frames with one ggplot function

#Isolate the N2 fluxes and reset the row numbers (which I'm not sure if is necessary but it looks better)
melted.N2 <- melted.all[melted.all$variable == "N2Flux", ]
rownames(melted.N2) <- NULL

#Isolating summer N2 fluxes
summer.N2 <- melted.N2[melted.N2$Season == "Summer", ]
rownames(summer.N2) <- NULL

#Code from Adam to double-check that geom_smooth is calculating the regression figure correctly. fitted.values is an automatic factor created when running the lm function. Need to insert z into the graph as a geom_smooth, and then delete it if it tracks the automatic geom_smooth-- so lines in the graphing code will be commented out.
test1 <- lm(melted.N2$value ~ poly(melted.N2$Age, 2))
z <- data.frame("Age" = melted.N2$Age, "fit" = test1$fitted.values)

test2 <- lm(summer.N2$value ~ poly(summer.N2$Age, 2))
z2 <- data.frame("Age" = summer.N2$Age, "fit" = test2$fitted.values)
...
(p12.N2 <- ggplot(data = melted.N2) +
    ...
    #geom_smooth(data = z, aes(Age, fit), method = "lm", formula = quad.formula) +
    #geom_smooth(data = z2, aes(Age, fit), method = "lm", formula = quad.formula) 
  )

#Creating new data frame and fake graph just to get a relevant legend

no.seasons <- mutate(melted.N2, Legend.Label = ifelse(Season == "Summer", "Summer", "Annual"))
no.seasons$Legend.Label <- factor(no.seasons$Legend.Label, levels = c("Summer", "Annual"))
...
#Plot of SOD only, using same general approach as above

melted.SOD <- melted.all[melted.all$variable == "SOD", ]
rownames(melted.SOD) <- NULL

#Checking linear fit
test3 <- lm(melted.SOD$value ~ poly(melted.SOD$Age, 2))
z3 <- data.frame("Age" = melted.SOD$Age, "fit" = test3$fitted.values)
...
p13.SOD <- ggplot(melted.SOD, aes(Age, value)) +
  ...
  theme(panel.background = element_rect(fill = "white", colour = NA),
       ...
       axis.text.y = element_text(size = 8, colour = "black")) +
  ylab(expression(paste("SOD (", mu, "mol ", O[2], " ", m^-2, " ", h^-1, ")")))
  #geom_smooth(data = z3, aes(Age, fit), method = "lm", formula = quad.formula)
...
#Bind together the plots using rbind. This will ensure that all the axes line up. Super important! Otherwise, things get wonky when g.arrange is used.
...
#Create a separate legend by extracting the legend from the plot created just for a legend.
...
tiff("Figures/EcoEng/rest-traj-N2-SOD.tif", width = 190, height = 100, units = "mm", res = 300)
...
###Restoration trajectory for SOM and BD

#Begin with SOM trajectory
melted.SOM <- melted.all[melted.all$variable == "PercentOM", ]
rownames(melted.SOM) <- NULL

#Check linear fit
test4 <- lm(melted.SOM$value ~ poly(melted.SOM$Age, 2))
z4 <- data.frame("Age" = melted.SOM$Age, "fit" = test4$fitted.values)

p14.SOM <- ggplot(melted.SOM, aes(Age, value)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm", formula = quad.formula, se = F,
              colour = "darkblue", size = 1) +
  geom_hline(yintercept = 0, size = 0.2, colour = "grey23") +
  ...
  #geom_smooth(data = z4, aes(Age, fit), method = "lm", formula = quad.formula)


#BD trajectory
#Melt sediment data since BD data only exists for one season
melted.sed <- melt(sed.data, measure.vars = c("PercentC", "PercentN", "CtoN", "BulkDensity", "SpringN2", "AnnualN2"))

#Isolate BD and add age as a numeric factor
melted.BD <- melted.sed[melted.sed$variable == "BulkDensity", ]
rownames(melted.BD) <- NULL
melted.BD <- add.age(melted.BD)
melted.BD$Age <- as.numeric(melted.BD$Age)

#Checking linear fit
test5 <- lm(melted.BD$value ~ poly(melted.BD$Age, 2))
z5 <- data.frame("Age" = melted.BD$Age, "fit" = test5$fitted.values)

p15.BD <- ggplot(melted.BD, aes(Age, value)) +
    geom_smooth(method = "lm", formula = quad.formula, se = F,
                colour = "darkblue", size = 1) +
    geom_point(size = 1) +
    geom_hline(yintercept = 0, size = 0.2, colour = "grey23") +
    ...
    #geom_smooth(data = z5, aes(Age, fit), method = "lm", formula = quad.formula)


#Using the grid package to put them all together.
grid.newpage()

tiff("Figures/EcoEng/rest-traj-SOM-BD.tif", width = 190, height = 100, units = "mm", res = 300)

grid.draw(rbind(ggplotGrob(p14.SOM), ggplotGrob(p15.BD), size = "last"))

dev.off()


  
###N2 vs SOD, with data color coded by season
#Major changes: got rid of space between x=0 and y axis, eliminated vertical line at x=0, changed scales, enlarged text, eliminated gridlines, increased space in legend

annual.data <- set.factors.basic(annual.data)

#Setting breaks to be used in dividing and labeling x axis below, using every_nth function found on SO
custom_breaks <- seq(0, 3000, 500)

#1 column

tiff("Figures/EcoEng/N2-flux-vs-SOD.tif", width = 90, height = 90, units = "mm", res = 300)

annual.data %>%
  ggplot(aes(SOD, N2Flux, colour = Season)) +
  geom_hline(yintercept = 0, size = 0.5, colour = "grey73") +
  #geom_vline(xintercept = 0, size = 0.7, colour = "grey73") +
  geom_point(size = 1.4) +
  theme(panel.background = element_rect(fill = "white", colour = NA),
